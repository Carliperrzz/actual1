...
async function startBot() {
  try {
    ...
    // 🚫 Anti "abrir/ler" mensagens: bloqueia qualquer tentativa de enviar recibos de leitura
    const blockReceipt = (name) => async (...args) => {
      try { console.log(`[RECEIPT-BLOCK] ${name}`, args?.[0]?.remoteJid || args?.[0] || ''); } catch (_) {}
      return;
    };
